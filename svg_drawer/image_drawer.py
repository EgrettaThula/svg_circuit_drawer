...
class ImageDrawer:
    """SvgImage drawer class called from circuit_drawer"""
    ...
    def __init__(
        ...
        self._style, def_font_ratio = load_style(None)
        ...
        self._figWpr = SvgImage(defs, style, self._rich_snips)
        self._figWpr.set_facecolor(self._style["bg"])
    ...
    def draw(self, filename=None, verbose=False):
        ...
        self._figWpr.set_xlim(xl, xr)
        self._figWpr.set_ylim(yb, yt)
        ...
        if self._scale != 1.0:
            self._figWpr.set_size_inches(base_fig_w * self._scale, base_fig_h * self._scale)
        ...
        # if "figwidth" style param set, use this to scale
            ...
            self._figWpr.set_size_inches(adj_fig_w, adj_fig_w * base_fig_h / base_fig_w)
            scale = adj_fig_w / base_fig_w

        # otherwise, display default size
        else:
            self._figWpr.set_size_inches(base_fig_w, base_fig_h)
        ...
        if filename:
            self._figWpr.savefig(filename, dpi=self._style["dpi"])

        return self._figWpr.draw()
        ...
        for fold_num in range(num_folds + 1):
            ...
            for qubit in self._qubits_dict.values():
                ...
                self._figWpr.text(
                    ...
                    clip_on=True,
                    zorder=PORDER_TEXT,
                    class_names=['qwire', 'bit'],
                )
                # draw the qubit wire
                self._line([self._x_offset, y], [xmax, y], zorder=PORDER_REGLINE, class_names=['qwire'])
    ...
    def _draw_ops(self, verbose=False):
        ...
        # Calibrations:
        if self._rich_snips:
            for name in self._calibrations:
                calibration = self._calibrations[name]
                for k in calibration:
                    fig = calibration[k].draw()
                    size = fig.get_size_inches() * fig.dpi
                    img_bio = BytesIO()
                    fig.savefig(img_bio, format="svg")
                    fig.clf()
                    self._figWpr.embed('calibration-' + name, img_bio.getvalue(), size[0], size[1])
    ...
    def _condition(self, node, cond_xy):
        ...
        for xy, m in zip(cond_xy, clbit_mask):
            if m == "1":
                if xy not in xy_plot:
                    ...
                    self._figWpr.add_patch(box, [node.name, 'ccontrol'])
                    xy_plot.append(xy)
                v_ind += 1
        ...
        self._figWpr.text(
            ...
            clip_on=True,
            zorder=PORDER_TEXT,
            class_names=['cwire', 'ccontrol'],
        )
        self._line(qubit_b, clbit_b, lc=self._style["cc"], ls=self._style["cline"], class_names=['cwire'])
    ...
    def _measure(self, node):
        ...
        self._figWpr.add_patch(arc, ['measure-symbol'])
        self._figWpr.plot(
            [qx, qx + 0.35 * WID],
        ...
        self._figWpr.add_patch(arrowhead, ['measure-arrow'])
        # target
    ...
    def _barrier(self, node):
        ...
        for xy in self._data[node]["q_xy"]:
            ...
            self._figWpr.add_patch(box, ['barrier'])
    ...
    def _gate(self, node, xy=None):
        ...
        snips = []
        if self._rich_snips:
            snips = self._get_snips(node)

        self._figWpr.add_patch(box, [node.name], snips)
    ...
    def _control_gate(self, node):
        ...
        self._line(qubit_b, qubit_t, lc=self._data[node]["lc"], class_names=[op.name])
    ...
    def _set_ctrl_bits(self, node, ctrl_state, num_ctrl_qubits, qbit):
        ...
        for i in range(num_ctrl_qubits):
            ...
            if text:
                if top and qlist[i] == min_ctbit:
                    ...
                    text_top = False
            self._ctrl_qubit(node.op, qbit[i], fc=fc_open_close, ec=ec, tc=tc, text=text, text_top=text_top)

    def _ctrl_qubit(self, op, xy, fc=None, ec=None, tc=None, text="", text_top=None):
        ...
        self._figWpr.add_patch(box, [op.name, 'control'])
    ...
    def _symmetric_gate(self, node, base_type):
        ...
        if not isinstance(op, ZGate) and isinstance(base_type, ZGate):
            num_ctrl_qubits = op.num_ctrl_qubits
            self._ctrl_qubit(op, xy[-1], fc=ec, ec=ec, tc=tc)
            self._line(qubit_b, qubit_t, lc=lc, zorder=PORDER_LINE + 1, class_names=[op.name])
    ...
    def _sidetext(self, node, xy, tc=None, text=""):
        ...
        self._figWpr.text(
            ...
            clip_on=True,
            zorder=PORDER_TEXT,
            class_names=[node.name, 'side-text'],
        )

    def _line(self, xy0, xy1, lc=None, ls=None, zorder=PORDER_LINE, class_names=[]):
    ...
    def _get_snips(self, node):
        snips = []
        if self._rich_snips:
            for name in self._calibrations:
                if name == node.name:
                    snips.append({ 'type': 'calibration', 'name': name })

            from qiskit import QuantumCircuit
            circ = QuantumCircuit(node.op.num_qubits, node.op.num_clbits)
            circ.append(node.op, range(node.op.num_qubits), range(node.op.num_clbits))
            if len(circ.decompose().count_ops()) > 1:
                fig = circ.decompose().draw('mpl', style = { 'backgroundcolor': '#ffffdd' })
                size = fig.get_size_inches() * fig.dpi
                img_bio = BytesIO()
                fig.savefig(img_bio, format="svg")
                fig.clf()
                self._figWpr.embed('decomposition-' + node.name, img_bio.getvalue(), size[0], size[1])
                snips.append({ 'type': 'decomposition', 'name': node.name })
        return snips
